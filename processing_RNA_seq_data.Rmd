...
go <- split_GO(x1)
go_prot <- split_GO(x1, 'gene_ontology_pfam')
...
annot_matrix[,3] <- rownames(data_sbt_1) %in% go_prot$transcript
...
rownames(annot_tbl_boo) <- rownames(data_sbt_1)
names(annot_tbl_boo) <- c('cv_prot','pfam','go_pfam', 'swissprot')
# complate cases of annotation:

annot_tbl_boo$complete <- rowSums(annot_tbl_boo)

dim(complete_annot <- annot_tbl_boo[annot_tbl_boo$complete == 4,])
...
dim(complete_cases_go_pep <- save_QA(data_sbt_complete_cases, go_prot,z))
...
dim(save_complete_nuc <- cbind(complete_cases_cv_pep, 
...
write.table(save_complete_nuc, file = paste0(dir, name, '_cv_pfam_goPfam_swissprot_nuc.txt'), sep="\t", 
            row.names = F, col.names = T)
```
...
```{r REMOVE Get uniprot and orf genome data}
annot_matrix <- matrix(ncol = 2, nrow = length(rownames(data_sbt_1)))
annot_matrix[,1] <- rownames(data_sbt_1) %in% go$transcript
annot_matrix[,2] <- rownames(data_sbt_1) %in%  blastn_genome$transcript


annot_tbl <- data.frame(annot_matrix)
names(annot_tbl) <- c('go', 'cv_prot')
...
annot_tbl_boo$complete <- rowSums(annot_tbl_boo)
dim(complete_annot <- annot_tbl[annot_tbl_boo$complete == 2,])

# And save complete cases:
dim(data_sbt_complete_cases <- data_sbt_1[rownames(data_sbt_1) %in% rownames(complete_annot), ])


dim(complete_cases_go <- save_QA(data_sbt_complete_cases, go))
