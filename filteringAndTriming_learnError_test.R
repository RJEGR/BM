# Ricardo Gomez
# filterAndTrim AND learnErrors test
...
# setdir ----
dir1 <- '/Users/cigom/metagenomics/COI/run11'
dir2 <- '/Users/cigom/metagenomics/COI/run012/mock_P68/raw/'
...
source("~/metagenomics/COI/run15/config.R")
...
data_path = dir2
out_path = paste0(dir1, '/dada2_asv')


## DADA2 pipeline ---- MOCK_HITS
fnFs <- sort(list.files(data_path, pattern = "*_R1_001.fastq.gz", full.names = T))
fnRs <- sort(list.files(data_path, pattern = "*_R2_001.fastq.gz", full.names = T))

source('~/Documents/GitHub/metagenomics/plotQualityProfile.R')

plotQP(list.files(data_path, pattern = 'fastq.gz', full.names = TRUE))

filtFs <- file.path(out_path, paste0("seqs_F_filt.fastq"))
filtRs <- file.path(out_path, paste0("seqs_R_filt.fastq"))


fat_trunclen <- c(300, 300)
fat_maxee <- c(2,2)
fat_trunQ <- 2
fat_minlen <- 20
...
# if use seqtab2 is neccesary to develop a way to transfor to matrix
...
# lens_df$Group <- '>300'
# 
# lens_df[lens_df$Length < 300, 'Group'] <- '<300'
# lens_df[lens_df$Length <= 200, 'Group'] <- '<=200'
# lens_df[lens_df$Length <= 100, 'Group'] <- '<=100'
